...
public class ProfiledPIDVController implements Sendable {
    ...
    private PIDVController controller;
     ...
     */
    public ProfiledPIDVController(double Kp, double Ki, double Kd, TrapezoidProfile.Constraints constraints) {
    ...
    public ProfiledPIDVController(double Kp, double Ki, double Kd, TrapezoidProfile.Constraints constraints, double period) {
        controller = new PIDVController(Kp, Ki, Kd, period);
    ...
    public double calculate(double measurement, double measurementVelocity) {
        if (controller.isContinuousInputEnabled()) {
            ...
            double setpointMinDistance = MathUtil.inputModulus(setpoint.position - measurement, -errorBound,
                    errorBound);
        ...
        setpoint = profile.calculate(getPeriod(), setpoint, goal);
        return controller.calculate(measurement, measurementVelocity, setpoint.position, setpoint.velocity);
    }
