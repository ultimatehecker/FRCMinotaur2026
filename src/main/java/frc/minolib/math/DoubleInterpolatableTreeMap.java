...
public class DoubleInterpolatableTreeMap<T> {
    ...
    public DoubleInterpolatableTreeMap(ArrayList<Pair<Double, Interpolatable<T>>> values) {
        set(values);
    }
    ...
    public void set(ArrayList<Pair<Double, Interpolatable<T>>> values) {
    ...
    public T get(double key) {
        ...
        if (topBound == null) {
            ...
            return bottomBound.getValue().interpolate(
                topBound.getValue(), (key - bottomBound.getKey()) / (topBound.getKey() - bottomBound.getKey())
            );
        }
    }
}